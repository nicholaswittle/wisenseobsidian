...
## 2026-07-20
- **[GATE B PARTIAL]**: Apex security — fixed claim/clock races + clock-out timezone (commit `9f723a5`, local): `claimOpenShift` now `.eq('staff','Open')`+`select()` guard; `clockIn` dedupes open entries; `clockOut` writes UTC. analyze clean, 7/7. **BLOCKED on RLS**: `supabase/migrations/README.md` says RLS migrations "applied to remote" but `docs/pillar0/MIGRATION_INVENTORY.md` says "SQL pending" — contradiction; SQL not in repo; can't verify live DB from here. Won't guess-write multi-tenant security SQL. Needs Nicholas to confirm live Supabase RLS state (or a session with DB access). Per-query org-scoping deferred behind RLS. See [[output/Launch Readiness — COMMS LINK and Apex 2026-07-20]].
