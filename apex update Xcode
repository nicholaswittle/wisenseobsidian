title: 2026-07-21 Daily Log
  ...
  **Goal was hit:** Apex now BUILDS, SIGNS, INSTALLS, and LAUNCHES on the iPhone in debug mode. Only remaining step
  ...
  - **Homebrew** at `/opt/homebrew/bin/brew` (not on non-login shell PATH — use the full path)
  - **Flutter 3.44.7** (`brew install --cask flutter`)
  - **CocoaPods 1.17.0 via brew** — `sudo gem install cocoapods` FAILED because the system Ruby is 2.6.10 and the `ffi` gem needs Ruby ≥3.0. Fix = `brew install cocoapods` (bundles modern Ruby). The `sudo gem`
  route in [[xcode_notes]] is a dead end on this Mac.
  - Apex cloned to `~/Developer/apex` (Flutter project is at the **repo root**, NOT nested `apex/apex` — [[xcode_notes]] is stale on that point)

  ### Device + signing
  - iPhone **"Nick Nasty"** (iPhone 15, iOS 26.5.2, UDID `00008120-000179CA3AE1A01E`) — paired, **Developer Mode enabled** (Settings → Privacy & Security; the menu item only appears after Xcode recognizes the
  device)
  - Signed by a **free Personal Team** (I started but never paid the $99 enrollment). Team `GWJ2K5U9T6`. Free-team installs re-sign every 7 days.
  ...
  Paste that in, click Commit changes, and it'll be in the vault and sync to your desktop. Once committed, the wikilinks ([[...]]) will resolve in Obsidian just like your other daily logs.

  A backup copy also lives on the Mac at ~/Developer/2026-07-21.md if you ever need it locally. That's everything for tonight — the note's ready to drop in whenever you want.
