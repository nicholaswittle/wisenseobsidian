...
| **4** | **Zettelkasten Graph Memory** | Navigates technical concepts via Wikilinks (`[[Note]]`). | Cross-linked **root notes** + `kepano/obsidian-skills` rules | 🟢 **Installed** (root notes; `wiki/` folder retired 2026-07-20) |
...
| **7** | **Manual Codification (on request)** | Reads raw sources, codifies into root notes, archives sources. | `raw/` $\rightarrow$ `raw/processed/` (manual; `sync_obsidian_vault.ps1` retired 2026-07-20) | 🟡 **Manual** (no auto-synthesis loop by design) |
...
| **10** | **Persona-Scoped Agent Steering** | Adapts system prompts dynamically based on active role directives. | Section 6 in `agents.md` (`Architect`, `Security`, `Travel`, `Flutter`) | 🟢 **100% Installed** |
| **11** | ~~**Automated Background Refresh**~~ | ~~Periodically syncs intake, conversion, Git commits.~~ | **Retired 2026-07-20** — `WiSenseVaultAutoSync` + sync scripts removed. Git commits are now manual/on-request. | ⚪ **Removed** (deliberate; vault is a curated static reference) |
